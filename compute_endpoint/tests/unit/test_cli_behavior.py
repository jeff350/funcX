...
def test_configure_ep_subscription_uuid_in_config(
    run_line, mock_command_ensure, make_endpoint_dir
):
    ep_name = "my-ep"
    ...
    run_line(f"configure {ep_name} --subscription-uuid {subscription_uuid}")

    with open(conf) as f:
        conf_dict = yaml.safe_load(f)

    assert conf_dict["subscription_uuid"] == subscription_uuid
