...
def test_start_endpoint_subscription_uuid_passthrough(mocker, fs):
    responses.add(  # 404 == we are verifying the POST, not the response
        responses.POST, _SVC_ADDY + "/v3/endpoints", json={}, status=404
    )
    ...
    ep = endpoint.Endpoint()
    ep_conf = Config()
    ep_conf.subscription_uuid = str(uuid.uuid4())
    ...
    assert req_json["subscription_uuid"] == ep_conf.subscription_uuid
